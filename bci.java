/*
 * Decompiled with CFR 0.152.
 */
public class bci
extends bbo {
    public bct a;
    public bct b;
    public bct c;
    public bct d;
    public bct e;
    public bct f;

    public bci(float f2) {
        this(f2, 0.0f, 64, 64);
    }

    public bci(float f2, float f3, int n2, int n3) {
        this.a = new bct(this).b(n2, n3);
        this.a.a(0.0f, 0.0f + f3, 0.0f);
        this.a.a(0, 0).a(-4.0f, -10.0f, -4.0f, 8, 10, 8, f2);
        this.f = new bct(this).b(n2, n3);
        this.f.a(0.0f, f3 - 2.0f, 0.0f);
        this.f.a(24, 0).a(-1.0f, -1.0f, -6.0f, 2, 4, 2, f2);
        this.a.a(this.f);
        this.b = new bct(this).b(n2, n3);
        this.b.a(0.0f, 0.0f + f3, 0.0f);
        this.b.a(16, 20).a(-4.0f, 0.0f, -3.0f, 8, 12, 6, f2);
        this.b.a(0, 38).a(-4.0f, 0.0f, -3.0f, 8, 18, 6, f2 + 0.5f);
        this.c = new bct(this).b(n2, n3);
        this.c.a(0.0f, 0.0f + f3 + 2.0f, 0.0f);
        this.c.a(44, 22).a(-8.0f, -2.0f, -2.0f, 4, 8, 4, f2);
        this.c.a(44, 22).a(4.0f, -2.0f, -2.0f, 4, 8, 4, f2);
        this.c.a(40, 38).a(-4.0f, 2.0f, -2.0f, 8, 4, 4, f2);
        this.d = new bct(this, 0, 22).b(n2, n3);
        this.d.a(-2.0f, 12.0f + f3, 0.0f);
        this.d.a(-2.0f, 0.0f, -2.0f, 4, 12, 4, f2);
        this.e = new bct(this, 0, 22).b(n2, n3);
        this.e.i = true;
        this.e.a(2.0f, 12.0f + f3, 0.0f);
        this.e.a(-2.0f, 0.0f, -2.0f, 4, 12, 4, f2);
    }

    @Override
    public void a(pk pk2, float f2, float f3, float f4, float f5, float f6, float f7) {
        this.a(f2, f3, f4, f5, f6, f7, pk2);
        this.a.a(f7);
        this.b.a(f7);
        this.d.a(f7);
        this.e.a(f7);
        this.c.a(f7);
    }

    @Override
    public void a(float f2, float f3, float f4, float f5, float f6, float f7, pk pk2) {
        this.a.g = f5 / 57.295776f;
        this.a.f = f6 / 57.295776f;
        this.c.d = 3.0f;
        this.c.e = -1.0f;
        this.c.f = -0.75f;
        this.d.f = ns.b(f2 * 0.6662f) * 1.4f * f3 * 0.5f;
        this.e.f = ns.b(f2 * 0.6662f + (float)Math.PI) * 1.4f * f3 * 0.5f;
        this.d.g = 0.0f;
        this.e.g = 0.0f;
    }
}

