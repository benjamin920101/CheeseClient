...
Versioned {
    ...
    public boolean isEnabled(Feature f2) {
        return f2.enabledIn(this._features);
    }

    public boolean isEnabled(StreamReadFeature f2) {
        return f2.mappedFeature().enabledIn(this._features);
    }
    ...
    public boolean getBooleanValue() throws IOException {
        JsonToken t2 = this.currentToken();
        if (t2 == JsonToken.VALUE_TRUE) {
            return true;
        }
        if (t2 == JsonToken.VALUE_FALSE) {
        ...
        throw new JsonParseException(this, String.format("Current token (%s) not of boolean type", new Object[]{t2})).withRequestPayload(this._requestPayload);
    }
    ...
    public int readBinaryValue(Base64Variant bv2, OutputStream out) throws IOException {
