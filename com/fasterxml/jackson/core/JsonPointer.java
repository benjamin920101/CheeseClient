...
public class JsonPointer {
    ...
    public static JsonPointer forPath(JsonStreamContext context, boolean includeRoot) {
        ...
        while (context != null) {
            if (context.inObject()) {
                ...
                int ix2 = context.getCurrentIndex();
                String ixStr = String.valueOf(ix2);
                tail = new JsonPointer(JsonPointer._fullPath(tail, ixStr), ixStr, ix2, tail);
            }
            context = context.getParent();
        }
        if (tail == null) {
    ...
    private static String _fullPath(JsonPointer tail, String segment) {
        if (tail == null) {
            StringBuilder sb2 = new StringBuilder(segment.length() + 1);
            sb2.append('/');
            JsonPointer._appendEscaped(sb2, segment);
            return sb2.toString();
        }
        String tailDesc = tail._asString;
        StringBuilder sb3 = new StringBuilder(segment.length() + 1 + tailDesc.length());
        sb3.append('/');
        JsonPointer._appendEscaped(sb3, segment);
        sb3.append(tailDesc);
        return sb3.toString();
    }

    private static void _appendEscaped(StringBuilder sb2, String segment) {
        int end = segment.length();
        for (int i2 = 0; i2 < end; ++i2) {
            char c2 = segment.charAt(i2);
            if (c2 == '/') {
                sb2.append("~1");
                continue;
            }
    ...
    protected static JsonPointer _parseQuotedTail(String input, int i2) {
        int end = input.length();
        StringBuilder sb2 = new StringBuilder(Math.max(16, end));
        if (i2 > 2) {
            sb2.append(input, 1, i2 - 1);
        }
        JsonPointer._appendEscape(sb2, input.charAt(i2++));
        while (i2 < end) {
            char c2 = input.charAt(i2);
            if (c2 == '/') {
                return new JsonPointer(input, sb2.toString(), JsonPointer._parseTail(input.substring(i2)));
            }
            if (c2 == '~' && ++i2 < end) {
                JsonPointer._appendEscape(sb2, input.charAt(i2++));
                continue;
        ...
        return new JsonPointer(input, sb2.toString(), EMPTY);
    }
