...
extends JsonGenerator {
    ...
    public void setCurrentValue(Object v2) {
        if (this._writeContext != null) {
            this._writeContext.setCurrentValue(v2);
        }
    }

    @Override
    public final boolean isEnabled(JsonGenerator.Feature f2) {
        return (this._features & f2.getMask()) != 0;
    }
    ...
    public JsonGenerator enable(JsonGenerator.Feature f2) {
        ...
        if ((mask & DERIVED_FEATURES_MASK) != 0) {
            if (f2 == JsonGenerator.Feature.WRITE_NUMBERS_AS_STRINGS) {
                this._cfgNumbersAsStrings = true;
            } else if (f2 == JsonGenerator.Feature.ESCAPE_NON_ASCII) {
                this.setHighestNonEscapedChar(127);
            } else if (f2 == JsonGenerator.Feature.STRICT_DUPLICATE_DETECTION && this._writeContext.getDupDetector() == null) {
    ...
    public JsonGenerator disable(JsonGenerator.Feature f2) {
        ...
        if ((mask & DERIVED_FEATURES_MASK) != 0) {
            ...
            } else if (f2 == JsonGenerator.Feature.STRICT_DUPLICATE_DETECTION) {
