...
extends ParserMinimalBase {
    ...
    @Override
    public JsonParser enable(JsonParser.Feature f2) {
        this._features |= f2.getMask();
        if (f2 == JsonParser.Feature.STRICT_DUPLICATE_DETECTION && this._parsingContext.getDupDetector() == null) {
    ...
    @Override
    public JsonParser disable(JsonParser.Feature f2) {
        this._features &= ~f2.getMask();
        if (f2 == JsonParser.Feature.STRICT_DUPLICATE_DETECTION) {
    ...
    protected void _checkStdFeatureChanges(int newFeatureFlags, int changedFeatures) {
        int f2 = JsonParser.Feature.STRICT_DUPLICATE_DETECTION.getMask();
        if ((changedFeatures & f2) != 0 && (newFeatureFlags & f2) != 0) {
    ...
    protected void _throwUnquotedSpace(int i2, String ctxtDesc) throws JsonParseException {
        if (!this.isEnabled(JsonParser.Feature.ALLOW_UNQUOTED_CONTROL_CHARS) || i2 > 32) {
            char c2 = (char)i2;
            String msg = "Illegal unquoted character (" + ParserBase._getCharDesc(c2) + "): has to be escaped using backslash to be included in " + ctxtDesc;
    ...
    protected static int[] growArrayBy(int[] arr2, int more) {
        if (arr2 == null) {
            return new int[more];
        }
