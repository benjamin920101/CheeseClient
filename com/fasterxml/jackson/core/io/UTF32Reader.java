...
extends Reader {
    ...
    public UTF32Reader(IOContext ctxt, InputStream in2, byte[] buf, int ptr, int len, boolean isBigEndian) {
        this._context = ctxt;
        this._in = in2;
        this._buffer = buf;
        this._ptr = ptr;
    ...
    public int read(char[] cbuf, int start, int len) throws IOException {
        ...
        while (outPtr < outEnd) {
            ...
            int ptr = this._ptr;
            if (this._bigEndian) {
                hi2 = this._buffer[ptr] << 8 | this._buffer[ptr + 1] & 0xFF;
                lo2 = (this._buffer[ptr + 2] & 0xFF) << 8 | this._buffer[ptr + 3] & 0xFF;
            } else {
                lo2 = this._buffer[ptr] & 0xFF | (this._buffer[ptr + 1] & 0xFF) << 8;
                hi2 = this._buffer[ptr + 2] & 0xFF | this._buffer[ptr + 3] << 8;
            }
            this._ptr += 4;
            if (hi2 != 0) {
                int ch = (hi2 &= 0xFFFF) - 1 << 16 | lo2;
                if (hi2 > 16) {
    ...
    private boolean loadMore(int available) throws IOException {
        ...
        if (available > 0) {
            ...
            this._ptr = 0;
            int n2 = count = this._in == null ? -1 : this._in.read(this._buffer);
            if (count < 1) {
                this._length = 0;
                if (count < 0) {
        ...
        while (this._length < 4) {
            int n3 = count = this._in == null ? -1 : this._in.read(this._buffer, this._length, this._buffer.length - this._length);
            if (count < 1) {
                if (count < 0) {
                    if (this._managedBuffers) {
                        this.freeBuffers();
