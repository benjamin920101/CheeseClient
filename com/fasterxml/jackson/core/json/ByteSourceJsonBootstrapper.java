...
public final class ByteSourceJsonBootstrapper {
    ...
    public ByteSourceJsonBootstrapper(IOContext ctxt, InputStream in2) {
        this._context = ctxt;
        this._in = in2;
    ...
    public static int skipUTF8BOM(DataInput input) throws IOException {
        ...
        b2 = input.readUnsignedByte();
        if (b2 != 191) {
            throw new IOException("Unexpected byte 0x" + Integer.toHexString(b2) + " following 0xEF 0xBB; should get 0xBF as part of UTF-8 BOM");
        }
        ...
        switch (enc.bits()) {
            ...
            case 16: {
                ...
                if (in2 == null) {
                    ...
                    in2 = new MergedStream(this._context, in2, this._inputBuffer, this._inputPtr, this._inputEnd);
                }
    ...
    public static MatchStrength hasJSONFormat(InputAccessor acc2) throws IOException {
        int ch;
        if (!acc2.hasMoreBytes()) {
            return MatchStrength.INCONCLUSIVE;
        }
        byte b2 = acc2.nextByte();
        if (b2 == -17) {
            if (!acc2.hasMoreBytes()) {
                return MatchStrength.INCONCLUSIVE;
            }
        ...
        if ((ch = ByteSourceJsonBootstrapper.skipSpace(acc2, b2)) < 0) {
            return MatchStrength.INCONCLUSIVE;
        }
        if (ch == 123) {
            ch = ByteSourceJsonBootstrapper.skipSpace(acc2);
            if (ch < 0) {
                return MatchStrength.INCONCLUSIVE;
            }
            if (ch == 34 || ch == 125) {
        ...
        if (ch == 110) {
            return ByteSourceJsonBootstrapper.tryMatch(acc2, "ull", strength);
        }
        if (ch == 116) {
            return ByteSourceJsonBootstrapper.tryMatch(acc2, "rue", strength);
        }
        if (ch == 102) {
            return ByteSourceJsonBootstrapper.tryMatch(acc2, "alse", strength);
        }
        return MatchStrength.NO_MATCH;
    }

    private static MatchStrength tryMatch(InputAccessor acc2, String matchStr, MatchStrength fullMatchStrength) throws IOException {
        int len = matchStr.length();
        for (int i2 = 0; i2 < len; ++i2) {
            if (!acc2.hasMoreBytes()) {
                return MatchStrength.INCONCLUSIVE;
            }
            if (acc2.nextByte() == matchStr.charAt(i2)) continue;
            return MatchStrength.NO_MATCH;
        }
        return fullMatchStrength;
    }

    private static int skipSpace(InputAccessor acc2) throws IOException {
        if (!acc2.hasMoreBytes()) {
            return -1;
        }
        return ByteSourceJsonBootstrapper.skipSpace(acc2, acc2.nextByte());
    }

    private static int skipSpace(InputAccessor acc2, byte b2) throws IOException {
        int ch;
        while ((ch = b2 & 0xFF) == 32 || ch == 13 || ch == 10 || ch == 9) {
            if (!acc2.hasMoreBytes()) {
                return -1;
            }
            b2 = acc2.nextByte();
