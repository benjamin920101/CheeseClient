...
extends JsonGeneratorImpl {
    ...
    protected final void _writePPFieldName(SerializableString name) throws IOException {
        ...
        boolean bl2 = addQuotes = !this._cfgUnqNames;
        if (addQuotes) {
            if (this._outputTail >= this._outputEnd) {
                this._flushBuffer();
            }
    ...
    private final void _writeUTF8Segment2(byte[] utf8, int offset, int len) throws IOException, JsonGenerationException {
        ...
        while (offset < len) {
            byte b2;
            byte ch;
            if ((ch = (b2 = utf8[offset++])) < 0 || escCodes[ch] == 0) {
                outputBuffer[outputPtr++] = b2;
                continue;
            }
            int escape = escCodes[ch];
            if (escape > 0) {
    ...
    private final int _readMore(InputStream in2, byte[] readBuffer, int inputPtr, int inputEnd, int maxRead) throws IOException {
        int length;
        int i2 = 0;
    ...
    protected final void _outputSurrogates(int surr1, int surr2) throws IOException {
        int c2 = this._decodeSurrogate(surr1, surr2);
        if (this._outputTail + 4 > this._outputEnd) {
            this._flushBuffer();
        }
        byte[] bbuf = this._outputBuffer;
        bbuf[this._outputTail++] = (byte)(0xF0 | c2 >> 18);
        bbuf[this._outputTail++] = (byte)(0x80 | c2 >> 12 & 0x3F);
        bbuf[this._outputTail++] = (byte)(0x80 | c2 >> 6 & 0x3F);
        bbuf[this._outputTail++] = (byte)(0x80 | c2 & 0x3F);
    }
    ...
    private int _writeGenericEscape(int charToEscape, int outputPtr) throws IOException {
        ...
        bbuf[outputPtr++] = 117;
        if (charToEscape > 255) {
            int hi2 = charToEscape >> 8 & 0xFF;
            bbuf[outputPtr++] = HEX_CHARS[hi2 >> 4];
            bbuf[outputPtr++] = HEX_CHARS[hi2 & 0xF];
            charToEscape &= 0xFF;
        } else {
            bbuf[outputPtr++] = 48;
            bbuf[outputPtr++] = 48;
