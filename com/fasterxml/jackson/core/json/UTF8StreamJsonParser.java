...
extends ParserBase {
    ...
    @Deprecated
    public UTF8StreamJsonParser(IOContext ctxt, int features, InputStream in2, ObjectCodec codec, ByteQuadsCanonicalizer sym, byte[] inputBuffer, int start, int end, boolean bufferRecyclable) {
        this(ctxt, features, in2, codec, sym, inputBuffer, start, end, 0, bufferRecyclable);
    }

    public UTF8StreamJsonParser(IOContext ctxt, int features, InputStream in2, ObjectCodec codec, ByteQuadsCanonicalizer sym, byte[] inputBuffer, int start, int end, int bytesPreProcessed, boolean bufferRecyclable) {
        super(ctxt, features);
        this._inputStream = in2;
    ...
    protected JsonToken _parsePosNumber(int c2) throws IOException {
        ...
        while (true) {
            ...
            if ((c2 = this._inputBuffer[this._inputPtr++] & 0xFF) < 48 || c2 > 57) break;
            ++intLen;
    ...
    protected JsonToken _parseNegNumber() throws IOException {
        ...
        if ((c2 = this._inputBuffer[this._inputPtr++] & 0xFF) <= 48) {
            if (c2 != 48) {
    ...
    private final JsonToken _parseNumber2(char[] outBuf, int outPtr, boolean negative, int intPartLength) throws IOException {
        while (true) {
            ...
            if ((c2 = this._inputBuffer[this._inputPtr++] & 0xFF) > 57 || c2 < 48) {
                if (c2 != 46 && c2 != 101 && c2 != 69) break;
                return this._parseFloat(outBuf, outPtr, c2, negative, intPartLength);
            }
    ...
    private final JsonToken _parseFloat(char[] outBuf, int outPtr, int c2, boolean negative, int integerPartLength) throws IOException {
        ...
        if (c2 == 101 || c2 == 69) {
            ...
            if ((c2 = this._inputBuffer[this._inputPtr++] & 0xFF) == 45 || c2 == 43) {
                if (outPtr >= outBuf.length) {
                    outBuf = this._textBuffer.finishCurrentSegment();
                    outPtr = 0;
                }
    ...
    private final void _finishString2(char[] outBuf, int outPtr) throws IOException {
        ...
        while (true) {
            ...
            block15: {
                ...
                while (ptr < max) {
                    if (codes[c2 = inputBuffer[ptr++] & 0xFF] == 0) {
                        outBuf[outPtr++] = (char)c2;
                        continue;
                    }
                    break block15;
    ...
    protected void _skipString() throws IOException {
        ...
        block6: while (true) {
            ...
            block10: {
                ...
                while (ptr < max) {
                    if (codes[c2 = inputBuffer[ptr++] & 0xFF] == 0) continue;
                    break block10;
                }
                this._inputPtr = ptr;
                continue;
    ...
    protected JsonToken _handleUnexpectedValue(int c2) throws IOException {
        ...
        if (Character.isJavaIdentifierStart(c2)) {
            this._reportInvalidToken("" + (char)c2, this._validJsonTokenList());
        }
    ...
    protected JsonToken _handleApos() throws IOException {
        ...
        block6: while (true) {
            ...
            while (true) {
                ...
                if ((c2 = inputBuffer[this._inputPtr++] & 0xFF) == 39 || codes[c2] != 0) break;
                outBuf[outPtr++] = (char)c2;
            }
            if (c2 == 39) break;
            switch (codes[c2]) {
    ...
    private final void _skipComment() throws IOException {
        ...
        if ((c2 = this._inputBuffer[this._inputPtr++] & 0xFF) == 47) {
            this._skipLine();
        } else if (c2 == 42) {
            this._skipCComment();
        } else {
    ...
    private final void _skipUtf8_2() throws IOException {
        ...
        if (((c2 = this._inputBuffer[this._inputPtr++]) & 0xC0) != 128) {
            this._reportInvalidOther(c2 & 0xFF, this._inputPtr);
        }
    }
    ...
    private final void _skipUtf8_4(int c2) throws IOException {
        byte d2;
        if (this._inputPtr >= this._inputEnd) {
            this._loadMoreGuaranteed();
        }
    ...
    protected void _reportInvalidToken(String matchedPart, String msg) throws IOException {
        ...
        while ((this._inputPtr < this._inputEnd || this._loadMore()) && Character.isJavaIdentifierPart(c2 = (char)this._decodeCharForError(i2 = this._inputBuffer[this._inputPtr++]))) {
            sb2.append(c2);
            if (sb2.length() < 256) continue;
            sb2.append("...");
            break;
        }
        this._reportError("Unrecognized token '%s': was expecting %s", sb2, msg);
    }
