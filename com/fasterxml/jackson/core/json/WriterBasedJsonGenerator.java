...
extends JsonGeneratorImpl {
    ...
    public WriterBasedJsonGenerator(IOContext ctxt, int features, ObjectCodec codec, Writer w2) {
        this(ctxt, features, codec, w2, '\"');
    }

    public WriterBasedJsonGenerator(IOContext ctxt, int features, ObjectCodec codec, Writer w2, char quoteChar) {
    ...
    public void writeString(Reader reader, int len) throws IOException {
        ...
        int n2 = toRead = len >= 0 ? len : Integer.MAX_VALUE;
    ...
    private void _writeString2(int len) throws IOException {
        ...
        block0: while (this._outputTail < end) {
            char c2;
            while ((c2 = this._outputBuffer[this._outputTail]) >= escLen || escCodes[c2] == 0) {
            ...
            char c3 = this._outputBuffer[this._outputTail++];
            this._prependOrWriteCharacterEscape(c3, escCodes[c3]);
        }
    }
    ...
    private void _writeSegment(int end) throws IOException {
        ...
        while (ptr < end) {
            char c2;
            while (((c2 = this._outputBuffer[ptr]) >= escLen || escCodes[c2] == 0) && ++ptr < end) {
            }
            ...
            start = this._prependOrWriteCharacterEscape(this._outputBuffer, ++ptr, end, c2, escCodes[c2]);
        }
    }
    ...
    private void _writeString(char[] text, int offset, int len) throws IOException {
        ...
        while (offset < len) {
            char c2;
            int start = offset;
            while (((c2 = text[offset]) >= escLen || escCodes[c2] == 0) && ++offset < len) {
            }
            ...
            if (offset >= len) break;
            char c3 = text[offset++];
            this._appendCharacterEscape(c3, escCodes[c3]);
        }
    }
    ...
    private void _writeStringASCII(int len, int maxNonEscaped) throws IOException, JsonGenerationException {
        ...
        while (this._outputTail < end) {
            ...
            block5: {
                do {
                    if ((c2 = this._outputBuffer[this._outputTail]) < escLimit) {
                        escCode = escCodes[c2];
                        if (escCode == 0) continue;
                    } else {
                        if (c2 <= maxNonEscaped) continue;
                        escCode = -1;
                    }
                    break block5;
                } while (++this._outputTail < end);
            ...
            ++this._outputTail;
            this._prependOrWriteCharacterEscape(c2, escCode);
        }
    }
    ...
    private void _writeSegmentASCII(int end, int maxNonEscaped) throws IOException, JsonGenerationException {
        ...
        while (ptr < end) {
            char c2;
            do {
                if ((c2 = this._outputBuffer[ptr]) < escLimit) {
                    escCode = escCodes[c2];
                    if (escCode == 0) continue;
                    break;
                }
            ...
            start = this._prependOrWriteCharacterEscape(this._outputBuffer, ++ptr, end, c2, escCode);
        }
    }
    ...
    private void _writeStringASCII(char[] text, int offset, int len, int maxNonEscaped) throws IOException, JsonGenerationException {
        ...
        while (offset < len) {
            ...
            if (offset >= len) break;
            ++offset;
            this._appendCharacterEscape(c2, escCode);
        }
    }
    ...
    private void _writeStringCustom(int len) throws IOException, JsonGenerationException {
        ...
        while (this._outputTail < end) {
            ...
            block7: {
                do {
                    if ((c2 = this._outputBuffer[this._outputTail]) < escLimit) {
                        escCode = escCodes[c2];
                        if (escCode == 0) continue;
                    } else if (c2 > maxNonEscaped) {
                        escCode = -1;
                    } else {
                        this._currentEscape = customEscapes.getEscapeSequence(c2);
            ...
            ++this._outputTail;
            this._prependOrWriteCharacterEscape((char)c2, escCode);
        }
    }
    ...
    private void _writeSegmentCustom(int end) throws IOException, JsonGenerationException {
        ...
        while (ptr < end) {
            ...
            start = this._prependOrWriteCharacterEscape(this._outputBuffer, ++ptr, end, (char)c2, escCode);
        }
    }
    ...
    private void _writeStringCustom(char[] text, int offset, int len) throws IOException, JsonGenerationException {
        ...
        while (offset < len) {
            ...
            if (offset >= len) break;
            ++offset;
            this._appendCharacterEscape((char)c2, escCode);
        }
    }
    ...
    private int _readMore(InputStream in2, byte[] readBuffer, int inputPtr, int inputEnd, int maxRead) throws IOException {
        int length;
        int i2 = 0;
        ...
        while ((length = maxRead - inputEnd) != 0) {
            int count = in2.read(readBuffer, inputEnd, length);
            if (count < 0) {
                return inputEnd;
            }
    ...
    private void _prependOrWriteCharacterEscape(char ch, int escCode) throws IOException, JsonGenerationException {
        ...
        if (escCode != -2) {
            if (this._outputTail >= 6) {
                ...
                if (ch > '\u00ff') {
                    int hi2 = ch >> 8 & 0xFF;
                    buf[++ptr] = HEX_CHARS[hi2 >> 4];
                    buf[++ptr] = HEX_CHARS[hi2 & 0xF];
                    ch = (char)(ch & 0xFF);
                } else {
                    buf[++ptr] = 48;
                    buf[++ptr] = 48;
            ...
            if (ch > '\u00ff') {
                int hi3 = ch >> 8 & 0xFF;
                int lo2 = ch & 0xFF;
                buf[10] = HEX_CHARS[hi3 >> 4];
                buf[11] = HEX_CHARS[hi3 & 0xF];
                buf[12] = HEX_CHARS[lo2 >> 4];
                buf[13] = HEX_CHARS[lo2 & 0xF];
    ...
    private int _prependOrWriteCharacterEscape(char[] buffer, int ptr, int end, char ch, int escCode) throws IOException, JsonGenerationException {
        ...
        if (escCode != -2) {
            if (ptr > 5 && ptr < end) {
                ...
                if (ch > '\u00ff') {
                    int hi2 = ch >> 8 & 0xFF;
                    buffer[ptr++] = HEX_CHARS[hi2 >> 4];
                    buffer[ptr++] = HEX_CHARS[hi2 & 0xF];
                    ch = (char)(ch & 0xFF);
                } else {
                    buffer[ptr++] = 48;
                    buffer[ptr++] = 48;
                ...
                if (ch > '\u00ff') {
                    int hi3 = ch >> 8 & 0xFF;
                    int lo2 = ch & 0xFF;
                    ent[10] = HEX_CHARS[hi3 >> 4];
                    ent[11] = HEX_CHARS[hi3 & 0xF];
                    ent[12] = HEX_CHARS[lo2 >> 4];
                    ent[13] = HEX_CHARS[lo2 & 0xF];
