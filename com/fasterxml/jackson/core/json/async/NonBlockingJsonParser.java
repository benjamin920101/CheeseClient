...
implements ByteArrayFeeder {
    ...
    protected final JsonToken _finishToken() throws IOException {
        switch (this._minorState) {
            ...
            case 41: {
                int c2 = this._decodeSplitEscaped(this._quoted32, this._quotedDigits);
                if (c2 < 0) {
                    return JsonToken.NOT_AVAILABLE;
                }
    ...
    protected JsonToken _finishErrorToken() throws IOException {
        while (this._inputPtr < this._inputEnd) {
            byte i2;
            char ch;
            if (Character.isJavaIdentifierPart(ch = (char)(i2 = this._inputBuffer[this._inputPtr++]))) {
    ...
    private int _decodeSplitEscaped(int value, int bytesRead) throws IOException {
        ...
        int c2 = this._inputBuffer[this._inputPtr++];
        if (bytesRead == -1) {
            switch (c2) {
                case 98: {
                    return 8;
        ...
        while (true) {
            int digit;
            if ((digit = CharTypes.charToHex(c2)) < 0) {
                this._reportUnexpectedChar(c2 & 0xFF, "expected a hex-digit for character escape sequence");
            }
            value = value << 4 | digit;
            if (++bytesRead == 4) {
                return value;
            ...
            c2 = this._inputBuffer[this._inputPtr++] & 0xFF;
        }
    }
    ...
    private final JsonToken _finishRegularString() throws IOException {
        ...
        block6: while (true) {
            ...
            while (true) {
                ...
                if (codes[c2 = inputBuffer[ptr++] & 0xFF] != 0) break;
                outBuf[outPtr++] = (char)c2;
            }
            if (c2 == 34) {
                this._inputPtr = ptr;
            ...
            if (ptr >= safeEnd) {
                this._inputPtr = ptr;
                this._textBuffer.setCurrentLength(outPtr);
                if (!this._decodeSplitMultiByte(c2, codes[c2], ptr < this._inputEnd)) {
            ...
            switch (codes[c2]) {
                case 1: {
                    this._inputPtr = ptr;
                    c2 = this._decodeFastCharEscape();
                    ptr = this._inputPtr;
                    break;
                }
                case 2: {
                    c2 = this._decodeUTF8_2(c2, this._inputBuffer[ptr++]);
                    break;
                }
                case 3: {
                    c2 = this._decodeUTF8_3(c2, this._inputBuffer[ptr++], this._inputBuffer[ptr++]);
                    break;
                }
                case 4: {
                    c2 = this._decodeUTF8_4(c2, this._inputBuffer[ptr++], this._inputBuffer[ptr++], this._inputBuffer[ptr++]);
    ...
    private final JsonToken _finishAposString() throws IOException {
        ...
        block6: while (true) {
            ...
            while (true) {
                if (ptr >= max) continue block6;
                if (codes[c2 = inputBuffer[ptr++] & 0xFF] != 0 && c2 != 34) break;
                if (c2 == 39) {
                    this._inputPtr = ptr;
                    this._textBuffer.setCurrentLength(outPtr);
                    return this._valueComplete(JsonToken.VALUE_STRING);
    ...
    private final boolean _decodeSplitMultiByte(int c2, int type, boolean gotNext) throws IOException {
        switch (type) {
            case 1: {
                c2 = this._decodeSplitEscaped(0, -1);
                if (c2 < 0) {
                    this._minorState = 41;
                    return false;
                }
            ...
            case 2: {
                if (gotNext) {
                    c2 = this._decodeUTF8_2(c2, this._inputBuffer[this._inputPtr++]);
                    this._textBuffer.append((char)c2);
                    return true;
                }
                this._minorState = 42;
            ...
            case 3: {
                c2 &= 0xF;
                if (gotNext) {
                    return this._decodeSplitUTF8_3(c2, 1, this._inputBuffer[this._inputPtr++]);
                }
                this._minorState = 43;
                this._pending32 = c2;
            ...
            case 4: {
                c2 &= 7;
                if (gotNext) {
                    return this._decodeSplitUTF8_4(c2, 1, this._inputBuffer[this._inputPtr++]);
                }
                this._pending32 = c2;
                this._pendingBytes = 1;
                this._minorState = 44;
    ...
    private final boolean _decodeSplitUTF8_4(int prev, int prevCount, int next) throws IOException {
        ...
        int c2 = (prev << 6 | next & 0x3F) - 65536;
        this._textBuffer.append((char)(0xD800 | c2 >> 10));
        c2 = 0xDC00 | c2 & 0x3FF;
        this._textBuffer.append((char)c2);
        return true;
    }
    ...
    private final int _decodeFastCharEscape() throws IOException {
        int digit;
        byte c2 = this._inputBuffer[this._inputPtr++];
        switch (c2) {
            case 98: {
                return 8;
            }
    ...
    private final int _decodeUTF8_2(int c2, int d2) throws IOException {
        if ((d2 & 0xC0) != 128) {
            this._reportInvalidOther(d2 & 0xFF, this._inputPtr);
        }
        return (c2 & 0x1F) << 6 | d2 & 0x3F;
    }

    private final int _decodeUTF8_3(int c2, int d2, int e2) throws IOException {
        c2 &= 0xF;
        if ((d2 & 0xC0) != 128) {
            this._reportInvalidOther(d2 & 0xFF, this._inputPtr);
        }
        c2 = c2 << 6 | d2 & 0x3F;
        if ((e2 & 0xC0) != 128) {
            this._reportInvalidOther(e2 & 0xFF, this._inputPtr);
        }
        return c2 << 6 | e2 & 0x3F;
    }

    private final int _decodeUTF8_4(int c2, int d2, int e2, int f2) throws IOException {
        if ((d2 & 0xC0) != 128) {
            this._reportInvalidOther(d2 & 0xFF, this._inputPtr);
        }
        c2 = (c2 & 7) << 6 | d2 & 0x3F;
        ...
        c2 = c2 << 6 | e2 & 0x3F;
        if ((f2 & 0xC0) != 128) {
            this._reportInvalidOther(f2 & 0xFF, this._inputPtr);
        }
        return (c2 << 6 | f2 & 0x3F) - 65536;
    }
}
