...
public final class ByteQuadsCanonicalizer {
    ...
    public String findName(int[] q2, int qlen) {
        if (qlen < 4) {
            switch (qlen) {
                case 3: {
                    return this.findName(q2[0], q2[1], q2[2]);
                }
                case 2: {
                    return this.findName(q2[0], q2[1]);
                }
                case 1: {
                    return this.findName(q2[0]);
                }
            }
            return "";
        }
        int hash = this.calcHash(q2, qlen);
        int offset = this._calcOffset(hash);
        int[] hashArea = this._hashArea;
        int len = hashArea[offset + 3];
        if (hash == hashArea[offset] && len == qlen && this._verifyLongName(q2, qlen, hashArea[offset + 1])) {
            return this._names[offset >> 2];
        }
        if (len == 0) {
            return null;
        ...
        if (hash == hashArea[offset2] && len2 == qlen && this._verifyLongName(q2, qlen, hashArea[offset2 + 1])) {
            return this._names[offset2 >> 2];
        }
        return this._findSecondary(offset, hash, q2, qlen);
    }

    private final int _calcOffset(int hash) {
        int ix2 = hash & this._hashSize - 1;
        return ix2 << 2;
    }
    ...
    private String _findSecondary(int origOffset, int hash, int[] q2, int qlen) {
        int offset;
        int[] hashArea = this._hashArea;
        int bucketSize = 1 << this._tertiaryShift;
        int end = offset + bucketSize;
        for (offset = this._tertiaryStart + (origOffset >> this._tertiaryShift + 2 << this._tertiaryShift); offset < end; offset += 4) {
            int len = hashArea[offset + 3];
            if (hash == hashArea[offset] && qlen == len && this._verifyLongName(q2, qlen, hashArea[offset + 1])) {
                return this._names[offset >> 2];
            }
            if (len != 0) continue;
            return null;
        ...
        for (offset = this._spilloverStart(); offset < this._spilloverEnd; offset += 4) {
            if (hash != hashArea[offset] || qlen != hashArea[offset + 3] || !this._verifyLongName(q2, qlen, hashArea[offset + 1])) continue;
            return this._names[offset >> 2];
        }
        return null;
    }

    private boolean _verifyLongName(int[] q2, int qlen, int spillOffset) {
        int[] hashArea = this._hashArea;
        int ix2 = 0;
        switch (qlen) {
            default: {
                return this._verifyLongName2(q2, qlen, spillOffset);
            }
            case 8: {
                if (q2[ix2++] != hashArea[spillOffset++]) {
                    return false;
                }
            }
            case 7: {
            ...
            case 5: {
                if (q2[ix2++] == hashArea[spillOffset++]) break;
                return false;
            }
            case 4: 
        }
        ...
        return q2[ix2++] == hashArea[spillOffset++];
    }

    private boolean _verifyLongName2(int[] q2, int qlen, int spillOffset) {
        int ix2 = 0;
        do {
            if (q2[ix2++] == this._hashArea[spillOffset++]) continue;
            return false;
        } while (ix2 < qlen);
        return true;
    }
    ...
    public String addName(String name, int[] q2, int qlen) {
        this._verifySharing();
        if (this._intern) {
            name = InternCache.instance.intern(name);
        }
        switch (qlen) {
            case 1: {
                int offset = this._findOffsetForAdd(this.calcHash(q2[0]));
                this._hashArea[offset] = q2[0];
                this._hashArea[offset + 3] = 1;
                break;
            }
            case 2: {
                int offset = this._findOffsetForAdd(this.calcHash(q2[0], q2[1]));
                this._hashArea[offset] = q2[0];
                this._hashArea[offset + 1] = q2[1];
                this._hashArea[offset + 3] = 2;
                break;
            }
            case 3: {
                int offset = this._findOffsetForAdd(this.calcHash(q2[0], q2[1], q2[2]));
                this._hashArea[offset] = q2[0];
                this._hashArea[offset + 1] = q2[1];
                this._hashArea[offset + 2] = q2[2];
                this._hashArea[offset + 3] = 3;
                break;
            }
            default: {
                ...
                this._hashArea[offset + 1] = longStart = this._appendLongName(q2, qlen);
    ...
    public int calcHash(int[] q2, int qlen) {
        if (qlen < 4) {
            throw new IllegalArgumentException();
        }
        int hash = q2[0] ^ this._seed;
        hash += hash >>> 9;
        hash += q2[1];
        hash += hash >>> 15;
        hash *= 33;
    ...
    private void rehash() {
        ...
        block5: for (int offset = 0; offset < end; offset += 4) {
            ...
            switch (len) {
                case 1: {
                    q2[0] = oldHashArea[offset];
                    this.addName(name, q2, 1);
                    continue block5;
                }
                case 2: {
                    q2[0] = oldHashArea[offset];
                    q2[1] = oldHashArea[offset + 1];
                    this.addName(name, q2, 2);
                    continue block5;
                }
                case 3: {
                    q2[0] = oldHashArea[offset];
                    q2[1] = oldHashArea[offset + 1];
                    q2[2] = oldHashArea[offset + 2];
                    this.addName(name, q2, 3);
                    continue block5;
                }
                default: {
                    if (len > q2.length) {
                    ...
                    System.arraycopy(oldHashArea, qoff, q2, 0, len);
                    this.addName(name, q2, len);
                }
            }
        }
