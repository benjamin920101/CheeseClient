...
public final class CharsToNameCanonicalizer {
    ...
    public String findSymbol(char[] buffer, int start, int len, int h2) {
        ...
        if (sym != null) {
            ...
            if ((b2 = this._buckets[index >> 1]) != null) {
                sym = b2.has(buffer, start, len);
                if (sym != null) {
                    return sym;
                }
                sym = this._findSymbol2(buffer, start, len, b2.next);
                if (sym != null) {
                    return sym;
                }
            }
    ...
    private String _findSymbol2(char[] buffer, int start, int len, Bucket b2) {
        while (b2 != null) {
            String sym = b2.has(buffer, start, len);
            if (sym != null) {
                return sym;
            }
            b2 = b2.next;
    ...
    private String _addSymbol(char[] buffer, int start, int len, int h2, int index) {
        ...
        if (this._symbols[index] == null) {
            ...
            int bix2 = index >> 1;
            Bucket newB = new Bucket(newSymbol, this._buckets[bix2]);
            int collLen = newB.length;
            if (collLen > 100) {
                this._handleSpillOverflow(bix2, newB, index);
            } else {
                this._buckets[bix2] = newB;
    ...
    private void rehash() {
        ...
        for (int i2 = 0; i2 < size; ++i2) {
            ...
            int bix2 = index >> 1;
            this._buckets[bix2] = newB = new Bucket(symbol, this._buckets[bix2]);
            maxColl = Math.max(maxColl, newB.length);
        }
        int bucketSize = size >> 1;
        for (int i3 = 0; i3 < bucketSize; ++i3) {
            Bucket b2 = oldBuckets[i3];
            while (b2 != null) {
                ++count;
                String symbol = b2.symbol;
                int index = this._hashToIndex(this.calcHash(symbol));
                if (this._symbols[index] == null) {
                    this._symbols[index] = symbol;
                } else {
                    Bucket newB;
                    int bix3 = index >> 1;
                    this._buckets[bix3] = newB = new Bucket(symbol, this._buckets[bix3]);
                    maxColl = Math.max(maxColl, newB.length);
                }
                b2 = b2.next;
            }
    ...
    protected void verifyInternalConsistency() {
        ...
        int size = this._symbols.length;
        for (int i2 = 0; i2 < size; ++i2) {
            String symbol = this._symbols[i2];
            if (symbol == null) continue;
            ++count;
        }
        int bucketSize = size >> 1;
        for (int i3 = 0; i3 < bucketSize; ++i3) {
            Bucket b2 = this._buckets[i3];
            while (b2 != null) {
                ++count;
                b2 = b2.next;
            }
