...
extends Name {
    ...
    public static NameN construct(String name, int hash, int[] q2, int qlen) {
        if (qlen < 4) {
        ...
        int q4 = q2[3];
        int rem = qlen - 4;
        int[] buf = rem > 0 ? Arrays.copyOfRange(q2, 4, qlen) : null;
        return new NameN(name, hash, q1, q22, q3, q4, buf, qlen);
    }
    ...
    private final boolean _equals2(int[] quads) {
        ...
        for (int i2 = 0; i2 < end; ++i2) {
            if (quads[i2 + 4] == this.q[i2]) continue;
            return false;
        }
        return true;
    }
