...
public final class TextBuffer {
        ...
        if (this._resultString == null) {
            if (this._resultArray != null) {
                ...
                if (segLen == 0) {
                    ...
                    StringBuilder sb2 = new StringBuilder(segLen + currLen);
                    if (this._segments != null) {
                        int len = this._segments.size();
                        for (int i2 = 0; i2 < len; ++i2) {
                            char[] curr = this._segments.get(i2);
    ...
    public int contentsToWriter(Writer w2) throws IOException {
        int len;
        if (this._resultArray != null) {
            w2.write(this._resultArray);
            return this._resultArray.length;
        }
        if (this._resultString != null) {
            w2.write(this._resultString);
            return this._resultString.length();
        }
        if (this._inputStart >= 0) {
            int len2 = this._inputLen;
            if (len2 > 0) {
                w2.write(this._inputBuffer, this._inputStart, len2);
            }
            return len2;
        }
        int total = 0;
        if (this._segments != null) {
            ...
            for (int i2 = 0; i2 < end; ++i2) {
                ...
                w2.write(curr, 0, currLen);
                total += currLen;
            }
        }
        if ((len = this._currentSize) > 0) {
            w2.write(this._currentSegment, 0, len);
            total += len;
        }
        return total;
    }
    ...
    public void append(char c2) {
        ...
        curr[this._currentSize++] = c2;
    }

    public void append(char[] c2, int start, int len) {
        int amount;
        if (this._inputStart >= 0) {
            this.unshare(len);
        }
        ...
        if (max >= len) {
            System.arraycopy(c2, start, curr, this._currentSize, len);
            this._currentSize += len;
            return;
        }
        if (max > 0) {
            System.arraycopy(c2, start, curr, this._currentSize, max);
            start += max;
            len -= max;
        }
        do {
            ...
            System.arraycopy(c2, start, this._currentSegment, 0, amount);
