...
public abstract class DatabindContext {
    ...
    public JavaType resolveSubType(JavaType baseType, String subClassName) throws JsonMappingException {
        if (subClassName.indexOf(60) > 0) {
            JavaType t2 = this.getTypeFactory().constructFromCanonical(subClassName);
            if (t2.isTypeOrSubTypeOf(baseType.getRawClass())) {
                return t2;
            }
        } else {
            Class<?> cls;
            try {
                ...
                throw this.invalidTypeIdException(baseType, subClassName, String.format("problem: (%s) %s", e3.getClass().getName(), ClassUtil.exceptionMessage(e3)));
            }
    ...
    public JavaType resolveAndValidateSubType(JavaType baseType, String subClass, PolymorphicTypeValidator ptv) throws JsonMappingException {
        ...
        try {
            ...
            throw this.invalidTypeIdException(baseType, subClass, String.format("problem: (%s) %s", e3.getClass().getName(), ClassUtil.exceptionMessage(e3)));
        }
        if (!baseType.isTypeOrSuperTypeOf(cls)) {
    ...
    public ObjectIdGenerator<?> objectIdGeneratorInstance(Annotated annotated, ObjectIdInfo objectIdInfo) throws JsonMappingException {
        ...
        ObjectIdGenerator<?> objectIdGenerator = gen = hi2 == null ? null : hi2.objectIdGeneratorInstance(config, annotated, implClass);
        if (gen == null) {
    ...
    public ObjectIdResolver objectIdResolverInstance(Annotated annotated, ObjectIdInfo objectIdInfo) {
        ...
        MapperConfig<?> config = this.getConfig();
        HandlerInstantiator hi2 = config.getHandlerInstantiator();
        ObjectIdResolver objectIdResolver = resolver = hi2 == null ? null : hi2.resolverIdGeneratorInstance(config, annotated, implClass);
        if (resolver == null) {
    ...
    public Converter<Object, Object> converterInstance(Annotated annotated, Object converterDef) throws JsonMappingException {
        ...
        MapperConfig<?> config = this.getConfig();
        HandlerInstantiator hi2 = config.getHandlerInstantiator();
        Converter converter = conv = hi2 == null ? null : hi2.converterInstance(config, annotated, converterClass);
        if (conv == null) {
