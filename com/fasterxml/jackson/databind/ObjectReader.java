...
Serializable {
    ...
    protected ObjectReader(ObjectReader base, JsonFactory f2) {
        this._config = (DeserializationConfig)base._config.with(MapperFeature.SORT_PROPERTIES_ALPHABETICALLY, f2.requiresPropertyOrdering());
        this._context = base._context;
        this._rootDeserializers = base._rootDeserializers;
        this._parserFactory = f2;
        this._valueType = base._valueType;
        this._rootDeserializer = base._rootDeserializer;
        this._valueToUpdate = base._valueToUpdate;
        this._schema = base._schema;
    ...
    protected ObjectReader _new(ObjectReader base, JsonFactory f2) {
        return new ObjectReader(base, f2);
    }
    ...
    public ObjectReader with(JsonNodeFactory f2) {
        return this._with(this._config.with(f2));
    }

    public ObjectReader with(JsonFactory f2) {
        if (f2 == this._parserFactory) {
            return this;
        }
        ObjectReader r2 = this._new(this, f2);
        if (f2.getCodec() == null) {
            f2.setCodec(r2);
        }
        return r2;
    ...
    public ObjectReader with(TimeZone tz2) {
        return this._with((DeserializationConfig)this._config.with(tz2));
    }
    ...
    public boolean isEnabled(JsonParser.Feature f2) {
        return this._parserFactory.isEnabled(f2);
    }
    ...
    @Override
    public <T> Iterator<T> readValues(JsonParser p2, TypeReference<T> valueTypeRef) throws IOException {
        this._assertNotNull("p", p2);
    ...
    public <T extends TreeNode> T readTree(JsonParser p2) throws IOException {
        this._assertNotNull("p", p2);
        return (T)this._bindAsTreeOrNull(p2);
    }

    @Override
    public void writeTree(JsonGenerator g2, TreeNode rootNode) {
    ...
    public JsonNode readTree(byte[] json, int offset, int len) throws IOException {
        ...
        return this._bindAndCloseAsTree(this._considerFilter(this._parserFactory.createParser(json, offset, len), false));
    }
    ...
    protected final JsonNode _bindAsTree(JsonParser p2) throws IOException {
        ...
        if (t2 == JsonToken.VALUE_NULL) {
            ...
            ctxt = this.createDeserializationContext(p2);
        } else {
            ctxt = this.createDeserializationContext(p2);
            JsonDeserializer<Object> deser = this._findTreeDeserializer(ctxt);
            resultNode = this._unwrapRoot ? (JsonNode)this._unwrapAndDeserialize(p2, ctxt, this._jsonNodeType(), deser) : (JsonNode)deser.deserialize(p2, ctxt);
        }
        if (this._config.isEnabled(DeserializationFeature.FAIL_ON_TRAILING_TOKENS)) {
            this._verifyNoTrailingTokens(p2, ctxt, this._jsonNodeType());
        }
        return resultNode;
    }

    protected final JsonNode _bindAsTreeOrNull(JsonParser p2) throws IOException {
        DefaultDeserializationContext ctxt;
        JsonNode resultNode;
        JsonToken t2;
        this._config.initialize(p2);
    ...
    protected JsonNode _detectBindAndCloseAsTree(InputStream in2) throws IOException {
        DataFormatReaders.Match match = this._dataFormatReaders.findFormat(in2);
        if (!match.hasMatch()) {
            this._reportUnkownFormat(this._dataFormatReaders, match);
        }
        JsonParser p2 = match.createParserWithMatch();
    ...
    protected InputStream _inputStream(File f2) throws IOException {
    ...
    protected final JavaType _jsonNodeType() {
        JavaType t2 = this._jsonNodeType;
        if (t2 == null) {
            this._jsonNodeType = t2 = this.getTypeFactory().constructType((Type)((Object)JsonNode.class));
        }
        return t2;
    }
