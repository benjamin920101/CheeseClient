...
implements Serializable {
    ...
    private SerializationConfig _withJsonWriteFeatures(FormatFeature ... features) {
        ...
        for (FormatFeature f2 : features) {
            ...
            newSet |= mask;
            newMask |= mask;
            if (!(f2 instanceof JsonWriteFeature) || (oldF = ((JsonWriteFeature)f2).mappedFeature()) == null) continue;
    ...
    public SerializationConfig withDefaultPrettyPrinter(PrettyPrinter pp2) {
        return this._defaultPrettyPrinter == pp2 ? this : new SerializationConfig(this, pp2);
    }

    public PrettyPrinter constructDefaultPrettyPrinter() {
        PrettyPrinter pp2 = this._defaultPrettyPrinter;
    ...
    public void initialize(JsonGenerator g2) {
        PrettyPrinter pp2;
        if (SerializationFeature.INDENT_OUTPUT.enabledIn(this._serFeatures) && g2.getPrettyPrinter() == null && (pp2 = this.constructDefaultPrettyPrinter()) != null) {
            g2.setPrettyPrinter(pp2);
        }
        ...
        if (mask != 0 || useBigDec) {
            ...
            if (useBigDec) {
                int f2 = JsonGenerator.Feature.WRITE_BIGDECIMAL_AS_PLAIN.getMask();
                newFlags |= f2;
                mask |= f2;
            }
            g2.overrideStdFeatures(newFlags, mask);
        }
        if (this._formatWriteFeaturesToChange != 0) {
            g2.overrideFormatFeatures(this._formatWriteFeatures, this._formatWriteFeaturesToChange);
        }
    }
    ...
    public final boolean isEnabled(SerializationFeature f2) {
        return (this._serFeatures & f2.getMask()) != 0;
    }

    public final boolean isEnabled(JsonGenerator.Feature f2, JsonFactory factory) {
        int mask = f2.getMask();
        if ((this._generatorFeaturesToChange & mask) != 0) {
            return (this._generatorFeatures & f2.getMask()) != 0;
        }
        return factory.isEnabled(f2);
    }
