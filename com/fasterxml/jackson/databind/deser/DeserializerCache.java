...
implements Serializable {
    ...
    protected JsonDeserializer<Object> _createAndCache2(DeserializationContext ctxt, DeserializerFactory factory, JavaType type) throws JsonMappingException {
        ...
        boolean bl2 = addToCache = !this._hasCustomHandlers(type) && deser.isCachable();
    ...
    protected JsonDeserializer<Object> findDeserializerFromAnnotation(DeserializationContext ctxt, Annotated ann2) throws JsonMappingException {
        Object deserDef = ctxt.getAnnotationIntrospector().findDeserializer(ann2);
        if (deserDef == null) {
            return null;
        }
        JsonDeserializer<Object> deser = ctxt.deserializerInstance(ann2, deserDef);
        return this.findConvertingDeserializer(ctxt, ann2, deser);
    }

    protected JsonDeserializer<Object> findConvertingDeserializer(DeserializationContext ctxt, Annotated a2, JsonDeserializer<Object> deser) throws JsonMappingException {
        Converter<Object, Object> conv = this.findConverter(ctxt, a2);
        if (conv == null) {
            return deser;
        }
        JavaType delegateType = conv.getInputType(ctxt.getTypeFactory());
    ...
    protected Converter<Object, Object> findConverter(DeserializationContext ctxt, Annotated a2) throws JsonMappingException {
        Object convDef = ctxt.getAnnotationIntrospector().findDeserializationConverter(a2);
        if (convDef == null) {
            return null;
        }
        return ctxt.converterInstance(a2, convDef);
    }

    private JavaType modifyTypeByAnnotation(DeserializationContext ctxt, Annotated a2, JavaType type) throws JsonMappingException {
        Object cdDef;
        ...
        if (type.isMapLikeType() && (keyType = type.getKeyType()) != null && keyType.getValueHandler() == null && (kdDef = intr.findKeyDeserializer(a2)) != null && (kd = ctxt.keyDeserializerInstance(a2, kdDef)) != null) {
            type = ((MapLikeType)type).withKeyValueHandler(kd);
            keyType = type.getKeyType();
        }
        if ((contentType = type.getContentType()) != null && contentType.getValueHandler() == null && (cdDef = intr.findContentDeserializer(a2)) != null) {
            ...
            if (cdDef instanceof JsonDeserializer) {
                ...
                if (cdClass != null) {
                    cd2 = ctxt.deserializerInstance(a2, cdClass);
                }
            }
            if (cd2 != null) {
        ...
        type = intr.refineDeserializationType(ctxt.getConfig(), a2, type);
        return type;
    }

    private boolean _hasCustomHandlers(JavaType t2) {
        if (t2.isContainerType()) {
            JavaType kt;
            JavaType ct2 = t2.getContentType();
            if (ct2 != null && (ct2.getValueHandler() != null || ct2.getTypeHandler() != null)) {
                return true;
            }
            if (t2.isMapLikeType() && (kt = t2.getKeyType()).getValueHandler() != null) {
                return true;
            }
        }
        return false;
