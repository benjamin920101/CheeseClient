...
public class CreatorCollector {
        ...
        if (this.verifyNonDup(creator, 7, explicit)) {
            if (properties.length > 1) {
                ...
                for (int i2 = 0; i2 < len; ++i2) {
                    ...
                    if (name.isEmpty() && properties[i2].getInjectableValueId() != null || (old = names.put(name, i2)) == null) continue;
                    throw new IllegalArgumentException(String.format("Duplicate creator property \"%s\" (index %s vs %d) for type %s ", name, old, i2, ClassUtil.nameOf(this._beanDesc.getBeanClass())));
                }
            }
    ...
    private JavaType _computeDelegateType(DeserializationContext ctxt, AnnotatedWithParams creator, SettableBeanProperty[] delegateArgs) throws JsonMappingException {
        ...
        DeserializationConfig config = ctxt.getConfig();
        JavaType baseType = creator.getParameterType(ix2);
        AnnotationIntrospector intr = config.getAnnotationIntrospector();
        if (intr != null) {
            AnnotatedParameter delegate = creator.getParameter(ix2);
