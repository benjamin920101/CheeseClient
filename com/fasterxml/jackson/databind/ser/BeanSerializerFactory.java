...
implements Serializable {
    ...
    public JsonSerializer<Object> createSerializer(SerializerProvider prov, JavaType origType) throws JsonMappingException {
        ...
        if (intr == null) {
            ...
            try {
                ...
                return (JsonSerializer)prov.reportBadTypeDefinition((BeanDescription)beanDesc, e2.getMessage(), new Object[0]);
            }
        }
        if (type == origType) {
            staticTyping = false;
    ...
    public TypeSerializer findPropertyTypeSerializer(JavaType baseType, SerializationConfig config, AnnotatedMember accessor) throws JsonMappingException {
        ...
        TypeResolverBuilder<?> b2 = ai2.findPropertyTypeResolver(config, accessor, baseType);
        if (b2 == null) {
            ...
            typeSer = b2.buildTypeSerializer(config, baseType, subtypes);
        }
        return typeSer;
    }
    ...
    public TypeSerializer findPropertyContentTypeSerializer(JavaType containerType, SerializationConfig config, AnnotatedMember accessor) throws JsonMappingException {
        ...
        AnnotationIntrospector ai2 = config.getAnnotationIntrospector();
        TypeResolverBuilder<?> b2 = ai2.findPropertyContentTypeResolver(config, accessor, containerType);
        if (b2 == null) {
            ...
            typeSer = b2.buildTypeSerializer(config, contentType, subtypes);
        }
        return typeSer;
    }
    ...
    protected JsonSerializer<Object> constructBeanOrAddOnSerializer(SerializerProvider prov, JavaType type, BeanDescription beanDesc, boolean staticTyping) throws JsonMappingException {
        ...
        try {
            ...
            return (JsonSerializer)prov.reportBadTypeDefinition(beanDesc, "Failed to construct BeanSerializer for %s: (%s) %s", beanDesc.getType(), e2.getClass().getName(), e2.getMessage());
        }
    ...
    protected List<BeanPropertyWriter> findBeanProperties(SerializerProvider prov, BeanDescription beanDesc, BeanSerializerBuilder builder) throws JsonMappingException {
        ...
        PropertyBuilder pb2 = this.constructPropertyBuilder(config, beanDesc);
        ...
        for (BeanPropertyDefinition property : properties) {
            ...
            if (accessor instanceof AnnotatedMethod) {
                result.add(this._constructWriter(prov, property, pb2, staticTyping, (AnnotatedMethod)accessor));
                continue;
            }
            result.add(this._constructWriter(prov, property, pb2, staticTyping, (AnnotatedField)accessor));
        }
        return result;
    }
    ...
    protected List<BeanPropertyWriter> filterBeanProperties(SerializationConfig config, BeanDescription beanDesc, List<BeanPropertyWriter> props) {
        ...
        if (ignorals != null && !(ignored = ignorals.findIgnoredForSerialization()).isEmpty()) {
            Iterator<BeanPropertyWriter> it2 = props.iterator();
            while (it2.hasNext()) {
                if (!ignored.contains(it2.next().getName())) continue;
                it2.remove();
            }
        }
        return props;
    ...
    protected void processViews(SerializationConfig config, BeanSerializerBuilder builder) {
        ...
        for (int i2 = 0; i2 < propCount; ++i2) {
            BeanPropertyWriter bpw2 = props.get(i2);
            Class<?>[] views = bpw2.getViews();
            ...
            ++viewsFound;
            filtered[i2] = this.constructFilteredBeanWriter(bpw2, views);
        }
    ...
    protected void removeIgnorableTypes(SerializationConfig config, BeanDescription beanDesc, List<BeanPropertyDefinition> properties) {
        ...
        Iterator<BeanPropertyDefinition> it2 = properties.iterator();
        while (it2.hasNext()) {
            BeanPropertyDefinition property = it2.next();
            AnnotatedMember accessor = property.getAccessor();
            if (accessor == null) {
                it2.remove();
                continue;
    ...
    protected List<BeanPropertyWriter> removeOverlappingTypeIds(SerializerProvider prov, BeanDescription beanDesc, BeanSerializerBuilder builder, List<BeanPropertyWriter> props) {
        ...
        block0: for (int i2 = 0; i2 < end; ++i2) {
            BeanPropertyWriter bpw2 = props.get(i2);
            TypeSerializer td = bpw2.getTypeSerializer();
            ...
            for (BeanPropertyWriter w2 : props) {
                if (w2 == bpw2 || !w2.wouldConflictWithName(typePropName)) continue;
                bpw2.assignTypeSerializer(null);
                continue block0;
            }
        }
        return props;
    ...
    protected BeanPropertyWriter _constructWriter(SerializerProvider prov, BeanPropertyDefinition propDef, PropertyBuilder pb2, boolean staticTyping, AnnotatedMember accessor) throws JsonMappingException {
        ...
        return pb2.buildWriter(prov, propDef, type, annotatedSerializer, typeSer, contentTypeSer, accessor, staticTyping);
    }
}
