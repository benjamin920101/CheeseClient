...
implements Serializable {
    ...
    protected DefaultSerializerProvider(SerializerProvider src, SerializationConfig config, SerializerFactory f2) {
        super(src, config, f2);
    }
    ...
    public JsonSerializer<Object> serializerInstance(Annotated annotated, Object serDef) throws JsonMappingException {
        ...
        if (serDef instanceof JsonSerializer) {
            ...
            JsonSerializer jsonSerializer = ser = (hi2 = this._config.getHandlerInstantiator()) == null ? null : hi2.serializerInstance(this._config, annotated, serClass);
            if (ser == null) {
    ...
    public Object includeFilterInstance(BeanPropertyDefinition forProperty, Class<?> filterClass) {
        ...
        Object object = filter = hi2 == null ? null : hi2.includeFilterInstance(this._config, forProperty, filterClass);
        if (filter == null) {
    ...
    public boolean includeFilterSuppressNulls(Object filter) throws JsonMappingException {
        ...
        try {
            ...
            String msg = String.format("Problem determining whether filter of type '%s' should filter out `null` values: (%s) %s", filter.getClass().getName(), t2.getClass().getName(), ClassUtil.exceptionMessage(t2));
    ...
    private IOException _wrapAsIOE(JsonGenerator g2, Exception e2) {
        if (e2 instanceof IOException) {
            return (IOException)e2;
        }
        String msg = ClassUtil.exceptionMessage(e2);
        if (msg == null) {
            msg = "[no message for " + e2.getClass().getName() + "]";
        }
        return new JsonMappingException((Closeable)g2, msg, (Throwable)e2);
    }
    ...
    extends DefaultSerializerProvider {
        ...
        protected Impl(SerializerProvider src, SerializationConfig config, SerializerFactory f2) {
            super(src, config, f2);
        }
