...
extends AsArraySerializerBase<Iterator<?>> {
    ...
    @Override
    public void serializeContents(Iterator<?> value, JsonGenerator g2, SerializerProvider provider) throws IOException {
        if (!value.hasNext()) {
            return;
        }
        JsonSerializer serializer = this._elementSerializer;
        if (serializer == null) {
            this._serializeDynamicContents(value, g2, provider);
            return;
        }
    ...
    protected void _serializeDynamicContents(Iterator<?> value, JsonGenerator g2, SerializerProvider provider) throws IOException {
        TypeSerializer typeSer = this._valueTypeSerializer;
