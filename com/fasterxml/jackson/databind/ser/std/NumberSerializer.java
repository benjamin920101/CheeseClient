    ...
    extends ToStringSerializerBase {
        ...
        public void serialize(Object value, JsonGenerator gen, SerializerProvider provider) throws IOException {
            ...
            if (gen.isEnabled(JsonGenerator.Feature.WRITE_BIGDECIMAL_AS_PLAIN)) {
                BigDecimal bd2 = (BigDecimal)value;
                if (!this._verifyBigDecimalRange(gen, bd2)) {
                    String errorMsg = String.format("Attempt to write plain `java.math.BigDecimal` (see JsonGenerator.Feature.WRITE_BIGDECIMAL_AS_PLAIN) with illegal scale (%d): needs to be between [-%d, %d]", bd2.scale(), 9999, 9999);
                    provider.reportMappingProblem(errorMsg, new Object[0]);
                }
                text = bd2.toPlainString();
            } else {
                text = value.toString();
            }
