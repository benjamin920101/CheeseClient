...
public final class ClassUtil {
    ...
    public static void closeOnFailAndThrowAsIOE(JsonGenerator g2, Exception fail) throws IOException {
    ...
    public static void closeOnFailAndThrowAsIOE(JsonGenerator g2, Closeable toClose, Exception fail) throws IOException {
        if (g2 != null) {
            g2.disable(JsonGenerator.Feature.AUTO_CLOSE_JSON_CONTENT);
            try {
                g2.close();
    ...
    public static String getTypeDescription(JavaType fullType) {
        ...
        StringBuilder sb2 = new StringBuilder(80).append('`');
        sb2.append(fullType.toCanonical());
    ...
    public static String exceptionMessage(Throwable t2) {
        if (t2 instanceof JsonProcessingException) {
            return ((JsonProcessingException)t2).getOriginalMessage();
        }
    ...
    public static void checkAndFixAccess(Member member, boolean force) {
        block3: {
            ...
            try {
                ...
                throw new IllegalArgumentException("Cannot access " + member + " (from class " + declClass.getName() + "; failed to set access: " + se2.getMessage());
            }
        }
    }
    ...
    public static final class Ctor {
        ...
        public int getParamCount() {
            int c2 = this._paramCount;
            if (c2 < 0) {
                this._paramCount = c2 = this._ctor.getParameterTypes().length;
            }
            return c2;
        }
    ...
    private static class EnumTypeLocator {
        ...
        private static Field locateField(Class<?> fromClass, String expectedName, Class<?> type) {
            Field[] fields;
            Field found = null;
            for (Field f2 : fields = ClassUtil.getDeclaredFields(fromClass)) {
                if (!expectedName.equals(f2.getName()) || f2.getType() != type) continue;
                found = f2;
                break;
            }
            if (found == null) {
                for (Field f2 : fields) {
                    if (f2.getType() != type) continue;
                    if (found != null) {
                        return null;
                    }
                    found = f2;
