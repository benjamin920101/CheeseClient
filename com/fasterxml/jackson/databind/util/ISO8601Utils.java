...
public class ISO8601Utils {
    ...
    @Deprecated
    public static String format(Date date, boolean millis, TimeZone tz2) {
        return ISO8601Utils.format(date, millis, tz2, Locale.US);
    }

    public static String format(Date date, boolean millis, TimeZone tz2, Locale loc) {
        int offset;
        GregorianCalendar calendar = new GregorianCalendar(tz2, loc);
        calendar.setTime(date);
        StringBuilder sb2 = new StringBuilder(30);
        sb2.append(String.format("%04d-%02d-%02dT%02d:%02d:%02d", calendar.get(1), calendar.get(2) + 1, calendar.get(5), calendar.get(11), calendar.get(12), calendar.get(13)));
        if (millis) {
            sb2.append(String.format(".%03d", calendar.get(14)));
        }
        if ((offset = tz2.getOffset(calendar.getTimeInMillis())) != 0) {
            int hours = Math.abs(offset / 60000 / 60);
            int minutes = Math.abs(offset / 60000 % 60);
            sb2.append(String.format("%c%02d:%02d", Character.valueOf(offset < 0 ? (char)'-' : '+'), hours, minutes));
        } else {
            sb2.append('Z');
        }
        return sb2.toString();
        ...
        try {
            ...
            if (timezoneIndicator == 'Z') {
                ...
                if ("+0000".equals(timezoneOffset) || "+00:00".equals(timezoneOffset)) {
                    ...
                    String act2 = timezone.getID();
                    if (!act2.equals(timezoneId) && !(cleaned = act2.replace(":", "")).equals(timezoneId)) {
