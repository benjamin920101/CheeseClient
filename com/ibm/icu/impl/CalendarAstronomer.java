...
public class CalendarAstronomer {
    ...
    private double trueAnomaly(double meanAnomaly, double eccentricity) {
        double delta;
        double E2 = meanAnomaly;
        do {
            delta = E2 - eccentricity * Math.sin(E2) - meanAnomaly;
            E2 -= delta / (1.0 - eccentricity * Math.cos(E2));
        } while (Math.abs(delta) > 1.0E-5);
        return 2.0 * Math.atan(Math.tan(E2 / 2.0) * Math.sqrt((1.0 + eccentricity) / (1.0 - eccentricity)));
    }
