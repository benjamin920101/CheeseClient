...
class Mdct {
    ...
    synchronized void backward(float[] in2, float[] out) {
        ...
        int A2 = n2;
        for (i2 = 0; i2 < n8; ++i2) {
            x2[xO++] = -in2[inO + 2] * this.trig[(A2 -= 2) + 1] - in2[inO] * this.trig[A2];
            x2[xO++] = in2[inO] * this.trig[A2 + 1] - in2[inO + 2] * this.trig[A2];
            inO += 4;
        }
        inO = n2 - 4;
        for (i2 = 0; i2 < n8; ++i2) {
            x2[xO++] = in2[inO] * this.trig[(A2 -= 2) + 1] + in2[inO + 2] * this.trig[A2];
            x2[xO++] = in2[inO] * this.trig[A2] - in2[inO + 2] * this.trig[A2 + 1];
            inO -= 4;
        }
    ...
    private float[] mdct_kernel(float[] x2, float[] w2, int n2, int n22, int n4, int n8) {
        ...
        for (int i3 = 0; i3 < n8; ++i3) {
            ...
            float wACE = wA * this.trig[C2];
            float wBCE = wB * this.trig[C2++];
            float wACO = wA * this.trig[C2];
            float wBCO = wB * this.trig[C2++];
