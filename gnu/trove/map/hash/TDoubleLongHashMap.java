...
Externalizable {
    ...
    public boolean forEachValue(TLongProcedure procedure) {
        ...
        while (i2-- > 0) {
            if (states[i2] != 1 || procedure.execute(values[i2])) continue;
            return false;
        }
        return true;
    }
    ...
    public long adjustOrPutValue(double key, long adjust_amount, long put_amount) {
        ...
        if (index < 0) {
            ...
            long l2 = this._values[n2] + adjust_amount;
            this._values[n2] = l2;
            newValue = l2;
            isNewMapping = false;
        } else {
    ...
    public boolean equals(Object other) {
        ...
        while (i2-- > 0) {
            ...
            long that_value;
            long this_value;
            if (states[i2] != 1 || (this_value = values[i2]) == (that_value = that.get(key = this._set[i2])) || this_value == this_no_entry_value || that_value == that_no_entry_value) continue;
            return false;
        }
        return true;
    }
    ...
    public void readExternal(ObjectInput in2) throws IOException, ClassNotFoundException {
        ...
        this.setUp(size);
        while (size-- > 0) {
            double key = in2.readDouble();
            long val = in2.readLong();
            this.put(key, val);
        }
    }
    ...
    implements TLongCollection {
        ...
        public boolean remove(long entry) {
            ...
            int i2 = values.length;
            while (i2-- > 0) {
                if (set[i2] == 0.0 || set[i2] == 2.0 || entry != values[i2]) continue;
                TDoubleLongHashMap.this.removeAt(i2);
                return true;
            }
            return false;
        }
        ...
        public boolean retainAll(long[] array) {
            ...
            int i2 = values.length;
            while (i2-- > 0) {
                if (states[i2] != 1 || Arrays.binarySearch(array, values[i2]) >= 0) continue;
                TDoubleLongHashMap.this.removeAt(i2);
                changed = true;
            }
            return changed;
        ...
        public boolean removeAll(Collection<?> collection) {
            ...
            for (Object element : collection) {
                long c2;
                if (!(element instanceof Long) || !this.remove(c2 = ((Long)element).longValue())) continue;
                changed = true;
            }
            return changed;
        }
    ...
    implements TDoubleSet {
        ...
        public boolean equals(Object other) {
            ...
            int i2 = TDoubleLongHashMap.this._states.length;
            while (i2-- > 0) {
                if (TDoubleLongHashMap.this._states[i2] != 1 || that.contains(TDoubleLongHashMap.this._set[i2])) continue;
                return false;
            }
            return true;
        }
        ...
        public int hashCode() {
            ...
            int i2 = TDoubleLongHashMap.this._states.length;
            while (i2-- > 0) {
                if (TDoubleLongHashMap.this._states[i2] != 1) continue;
                hashcode += HashFunctions.hash(TDoubleLongHashMap.this._set[i2]);
            }
            return hashcode;
        }
