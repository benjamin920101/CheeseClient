...
public class RenderManager {
    ...
    public <T extends Entity> Render<T> getEntityRenderObject(Entity entityIn) {
        if (entityIn instanceof AbstractClientPlayer) {
            String s2 = ((AbstractClientPlayer)entityIn).getSkinType();
            RenderPlayer renderplayer = this.skinMap.get(s2);
    ...
    public void cacheActiveRenderInfo(World worldIn, FontRenderer textRendererIn, Entity livingPlayerIn, Entity pointedEntityIn, GameSettings optionsIn, float partialTicks) {
        ...
        if (livingPlayerIn instanceof EntityLivingBase && ((EntityLivingBase)livingPlayerIn).isPlayerSleeping()) {
            ...
            if (Reflector.callBoolean(block, Reflector.ForgeBlock_isBed, iblockstate, worldIn, new BlockPos(livingPlayerIn), (EntityLivingBase)livingPlayerIn)) {
                ...
                int i2 = enumfacing.getHorizontalIndex();
                this.playerViewY = i2 * 90 + 180;
                this.playerViewX = 0.0f;
            } else if (block == Blocks.bed) {
                int j2 = iblockstate.getValue(BlockBed.FACING).getHorizontalIndex();
                this.playerViewY = j2 * 90 + 180;
                this.playerViewX = 0.0f;
            }
        } else {
    ...
    public boolean renderEntityStatic(Entity entity, float partialTicks, boolean hideDebugBox) {
        ...
        return this.doRenderEntity(entity, d0 - this.renderPosX, d1 - this.renderPosY, d2 - this.renderPosZ, f2, partialTicks, hideDebugBox);
    }
    ...
    public void renderWitherSkull(Entity entityIn, float partialTicks) {
        ...
        if (render != null && this.renderEngine != null) {
            int i2 = entityIn.getBrightnessForRender(partialTicks);
            int j2 = i2 % 65536;
            int k2 = i2 / 65536;
            OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, (float)j2 / 1.0f, (float)k2 / 1.0f);
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
    ...
    public boolean doRenderEntity(Entity entity, double x2, double y2, double z2, float entityYaw, float partialTicks, boolean hideDebugBox) {
        ...
        try {
            ...
            if (render != null && this.renderEngine != null) {
                try {
                    ...
                    render.doRender(entity, x2, y2, z2, entityYaw, partialTicks);
                }
                ...
                try {
                    if (!this.renderOutlines) {
                        render.doRenderShadowAndFire(entity, x2, y2, z2, entityYaw, partialTicks);
                    }
                }
            ...
            crashreportcategory1.addCrashSection("Location", CrashReportCategory.getCoordinateInfo(x2, y2, z2));
    ...
    private void renderDebugBoundingBox(Entity entityIn, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        if (!Shaders.isShadowPass) {
            ...
            AxisAlignedBB axisalignedbb1 = new AxisAlignedBB(axisalignedbb.minX - entityIn.posX + x2, axisalignedbb.minY - entityIn.posY + y2, axisalignedbb.minZ - entityIn.posZ + z2, axisalignedbb.maxX - entityIn.posX + x2, axisalignedbb.maxY - entityIn.posY + y2, axisalignedbb.maxZ - entityIn.posZ + z2);
            RenderGlobal.drawOutlinedBoundingBox(axisalignedbb1, 255, 255, 255, 255);
            if (entityIn instanceof EntityLivingBase) {
                float f1 = 0.01f;
                RenderGlobal.drawOutlinedBoundingBox(new AxisAlignedBB(x2 - (double)f2, y2 + (double)entityIn.getEyeHeight() - (double)0.01f, z2 - (double)f2, x2 + (double)f2, y2 + (double)entityIn.getEyeHeight() + (double)0.01f, z2 + (double)f2), 255, 0, 0, 255);
            }
            Tessellator tessellator = Tessellator.getInstance();
            WorldRenderer worldrenderer = tessellator.getWorldRenderer();
            Vec3 vec3 = entityIn.getLook(partialTicks);
            worldrenderer.begin(3, DefaultVertexFormats.POSITION_COLOR);
            worldrenderer.pos(x2, y2 + (double)entityIn.getEyeHeight(), z2).color(0, 0, 255, 255).endVertex();
            worldrenderer.pos(x2 + vec3.xCoord * 2.0, y2 + (double)entityIn.getEyeHeight() + vec3.yCoord * 2.0, z2 + vec3.zCoord * 2.0).color(0, 0, 255, 255).endVertex();
            tessellator.draw();
    ...
    public double getDistanceToCamera(double x2, double y2, double z2) {
        double d0 = x2 - this.viewerPosX;
        double d1 = y2 - this.viewerPosY;
        double d2 = z2 - this.viewerPosZ;
