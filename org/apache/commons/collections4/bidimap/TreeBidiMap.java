...
Serializable {
    ...
    public void putAll(Map<? extends K, ? extends V> map) {
        for (Map.Entry<K, V> e2 : map.entrySet()) {
            this.put((K)((Comparable)e2.getKey()), (V)((Comparable)e2.getValue()));
        }
    }
    ...
    private void copyColor(Node<K, V> from, Node<K, V> to2, DataElement dataElement) {
        if (to2 != null) {
            if (from == null) {
                ((Node)to2).setBlack(dataElement);
            } else {
                ((Node)to2).copyColor((Node)from, dataElement);
            }
        }
    }
    ...
    private void doRedBlackInsert(Node<K, V> insertedNode, DataElement dataElement) {
        ...
        while (currentNode != null && currentNode != this.rootNode[dataElement.ordinal()] && TreeBidiMap.isRed(((Node)currentNode).getParent(dataElement), dataElement)) {
            ...
            if (((Node)currentNode).isLeftChild(dataElement)) {
                y2 = this.getRightChild(this.getGrandParent(currentNode, dataElement), dataElement);
                if (TreeBidiMap.isRed(y2, dataElement)) {
                    TreeBidiMap.makeBlack(this.getParent(currentNode, dataElement), dataElement);
                    TreeBidiMap.makeBlack(y2, dataElement);
            ...
            y2 = this.getLeftChild(this.getGrandParent(currentNode, dataElement), dataElement);
            if (TreeBidiMap.isRed(y2, dataElement)) {
                TreeBidiMap.makeBlack(this.getParent(currentNode, dataElement), dataElement);
                TreeBidiMap.makeBlack(y2, dataElement);
                TreeBidiMap.makeRed(this.getGrandParent(currentNode, dataElement), dataElement);
    ...
    private void swapPosition(Node<K, V> x2, Node<K, V> y2, DataElement dataElement) {
        boolean yWasLeftChild;
        Node xFormerParent = ((Node)x2).getParent(dataElement);
        Node xFormerLeftChild = ((Node)x2).getLeft(dataElement);
        Node xFormerRightChild = ((Node)x2).getRight(dataElement);
        Node yFormerParent = ((Node)y2).getParent(dataElement);
        Node yFormerLeftChild = ((Node)y2).getLeft(dataElement);
        Node yFormerRightChild = ((Node)y2).getRight(dataElement);
        boolean xWasLeftChild = ((Node)x2).getParent(dataElement) != null && x2 == ((Node)((Node)x2).getParent(dataElement)).getLeft(dataElement);
        boolean bl2 = yWasLeftChild = ((Node)y2).getParent(dataElement) != null && y2 == ((Node)((Node)y2).getParent(dataElement)).getLeft(dataElement);
        if (x2 == yFormerParent) {
            ((Node)x2).setParent((Node)y2, dataElement);
            if (yWasLeftChild) {
                ((Node)y2).setLeft((Node)x2, dataElement);
                ((Node)y2).setRight(xFormerRightChild, dataElement);
            } else {
                ((Node)y2).setRight((Node)x2, dataElement);
                ((Node)y2).setLeft(xFormerLeftChild, dataElement);
            }
        } else {
            ((Node)x2).setParent(yFormerParent, dataElement);
            if (yFormerParent != null) {
                if (yWasLeftChild) {
                    yFormerParent.setLeft((Node)x2, dataElement);
                } else {
                    yFormerParent.setRight((Node)x2, dataElement);
                }
            }
            ((Node)y2).setLeft(xFormerLeftChild, dataElement);
            ((Node)y2).setRight(xFormerRightChild, dataElement);
        ...
        if (y2 == xFormerParent) {
            ((Node)y2).setParent((Node)x2, dataElement);
            if (xWasLeftChild) {
                ((Node)x2).setLeft((Node)y2, dataElement);
                ((Node)x2).setRight(yFormerRightChild, dataElement);
            } else {
                ((Node)x2).setRight((Node)y2, dataElement);
                ((Node)x2).setLeft(yFormerLeftChild, dataElement);
            }
        } else {
            ((Node)y2).setParent(xFormerParent, dataElement);
            if (xFormerParent != null) {
                if (xWasLeftChild) {
                    xFormerParent.setLeft((Node)y2, dataElement);
                } else {
                    xFormerParent.setRight((Node)y2, dataElement);
                }
            }
            ((Node)x2).setLeft(yFormerLeftChild, dataElement);
            ((Node)x2).setRight(yFormerRightChild, dataElement);
        }
        if (((Node)x2).getLeft(dataElement) != null) {
            ((Node)((Node)x2).getLeft(dataElement)).setParent((Node)x2, dataElement);
        }
        if (((Node)x2).getRight(dataElement) != null) {
            ((Node)((Node)x2).getRight(dataElement)).setParent((Node)x2, dataElement);
        }
        if (((Node)y2).getLeft(dataElement) != null) {
            ((Node)((Node)y2).getLeft(dataElement)).setParent((Node)y2, dataElement);
        }
        if (((Node)y2).getRight(dataElement) != null) {
            ((Node)((Node)y2).getRight(dataElement)).setParent((Node)y2, dataElement);
        }
        ((Node)x2).swapColors((Node)y2, dataElement);
        if (this.rootNode[dataElement.ordinal()] == x2) {
            this.rootNode[dataElement.ordinal()] = y2;
        } else if (this.rootNode[dataElement.ordinal()] == y2) {
            this.rootNode[dataElement.ordinal()] = x2;
        }
    }

    private static void checkNonNullComparable(Object o2, DataElement dataElement) {
        if (o2 == null) {
    ...
    private boolean doEquals(Object obj, DataElement dataElement) {
        ...
        if (this.nodeCount > 0) {
            try {
                MapIterator<?, ?> it2 = this.getMapIterator(dataElement);
                while (it2.hasNext()) {
                    Object key = it2.next();
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        for (int i2 = 0; i2 < size; ++i2) {
            Comparable k2 = (Comparable)stream.readObject();
            Comparable v2 = (Comparable)stream.readObject();
            this.put((K)k2, (V)v2);
        }
    }
    ...
    implements OrderedBidiMap<V, K> {
        ...
        public void putAll(Map<? extends V, ? extends K> map) {
            for (Map.Entry e2 : map.entrySet()) {
                this.put((V)((Comparable)e2.getKey()), (K)((Comparable)e2.getValue()));
            }
        }
    ...
    KeyValue<K, V> {
        ...
        private void swapColors(Node<K, V> node, DataElement dataElement) {
            ...
            this.blackColor[n2] = this.blackColor[n2] ^ node.blackColor[dataElement.ordinal()];
            int n3 = dataElement.ordinal();
            node.blackColor[n3] = node.blackColor[n3] ^ this.blackColor[dataElement.ordinal()];
            int n4 = dataElement.ordinal();
            this.blackColor[n4] = this.blackColor[n4] ^ node.blackColor[dataElement.ordinal()];
        }
    ...
    extends View<V> {
        ...
        public boolean remove(Object o2) {
            return TreeBidiMap.this.doRemoveValue(o2) != null;
        }
    }
    ...
    extends View<K> {
        ...
        @Override
        public boolean remove(Object o2) {
            return TreeBidiMap.this.doRemoveKey(o2) != null;
        }
    }
