    ...
    static class AVLNode<E> {
        ...
        private AVLNode<E> addAll(AVLNode<E> otherTree, int currentSize) {
            ...
            if (otherTree.height > this.height) {
                ...
                AVLNode<E> s2 = otherTree;
                int sAbsolutePosition = s2.relativePosition + currentSize;
                int sParentAbsolutePosition = 0;
                while (s2 != null && s2.height > this.getHeight(leftSubTree)) {
                    sParentAbsolutePosition = sAbsolutePosition;
                    sAncestors.push(s2);
                    s2 = s2.left;
                    if (s2 == null) continue;
                    sAbsolutePosition += s2.relativePosition;
                }
                super.setLeft(leftSubTree, null);
                super.setRight(s2, otherTreeMin);
                if (leftSubTree != null) {
                ...
                if (s2 != null) {
                    ...
                    s2.relativePosition = sAbsolutePosition - currentSize + 1;
                }
            ...
            ArrayDeque<AVLNode<E>> sAncestors = new ArrayDeque<AVLNode<E>>();
            AVLNode<E> s3 = this;
            int sAbsolutePosition = s3.relativePosition;
            int sParentAbsolutePosition = 0;
            while (s3 != null && s3.height > this.getHeight(otherTree)) {
                sParentAbsolutePosition = sAbsolutePosition;
                sAncestors.push(s3);
                s3 = s3.right;
                if (s3 == null) continue;
                sAbsolutePosition += s3.relativePosition;
            }
            super.setRight(otherTree, null);
            super.setLeft(s3, maxNode);
            if (otherTree != null) {
            ...
            if (s3 != null) {
                ...
                s3.relativePosition = sAbsolutePosition - currentSize;
            }
            otherTreeMin.relativePosition = currentSize - sParentAbsolutePosition;
            s3 = otherTreeMin;
            while (!sAncestors.isEmpty()) {
                AVLNode sAncestor = (AVLNode)sAncestors.pop();
                sAncestor.setRight(s3, null);
                s3 = sAncestor.balance();
            }
            return s3;
        }
