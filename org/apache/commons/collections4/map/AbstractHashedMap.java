...
implements IterableMap<K, V> {
    ...
    public boolean containsValue(Object value) {
        if (value == null) {
            HashEntry<K, V>[] arr$ = this.data;
            int len$ = arr$.length;
            for (int i$ = 0; i$ < len$; ++i$) {
                HashEntry<K, V> element;
                HashEntry<K, V> entry = element = arr$[i$];
                while (entry != null) {
    ...
    protected void ensureCapacity(int newCapacity) {
        ...
        if (this.size == 0) {
            ...
            for (int i2 = oldCapacity - 1; i2 >= 0; --i2) {
                HashEntry next;
                HashEntry<K, V> entry = oldEntries[i2];
    ...
    public int hashCode() {
        int total = 0;
        Iterator<Map.Entry<K, V>> it2 = this.createEntrySetIterator();
        while (it2.hasNext()) {
            total += it2.next().hashCode();
        }
        return total;
    }
    ...
    extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains(Object entry) {
            if (entry instanceof Map.Entry) {
                Map.Entry e2 = (Map.Entry)entry;
                HashEntry<K, V> match = this.parent.getEntry(e2.getKey());
                return match != null && match.equals(e2);
            }
            return false;
        }
